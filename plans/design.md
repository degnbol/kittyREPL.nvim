...
```lua
local wins = {}   -- [win] = { cmd = "radian", pending = "radian"|nil, sent = {} }

M.attach(win, hint)  -- bind buffer -> win, resolve + cache cmd, returns cmd|nil
M.current()          -- -> win|nil, cmd|nil   (validated: one `kitty @ ls --match id:`)
M.cmd(win)           -- cached program name, resolving lazily if pending
M.forget(win)        -- drop per-window state
```
...
is a list whose last element shifts for benign reasons — `detect.lua:41-43`
...
the program. `repl.forget(win)` in `commands.new` is the invalidation.
...
- **filetype-keyed** — chosen from the file being edited: `exclude`, `command`,
  `command_count`, `match.detect`, `iterate`
...
`os.execute("true")` is `0`, `os.execute("false")` is `256`. The `== 0` checks at
`kitty.lua:71` and `kitty.lua:214` are correct today and must become
...
**No kitty and no seam needed:** every `config.match.detect.*` function;
...
**Seam needed:** `kitty.send` dispatch, `repl.attach` resolution, `replCheck`.
