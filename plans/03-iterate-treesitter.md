...
04's removal of `vim.b.repl_cmd` does not reach it. Three mechanical overlaps:
...
- Its rewritten commands call `kitty.run(torun, true)`. Task 05 gives `kitty.run`
  an explicit `win` first parameter, so whichever lands second updates those two
  call sites.
- It adds a filetype-keyed `iterate` table. Task 02 writes the key-axis comment
  block in `config.lua`; add `iterate` to it whenever the later of the two lands.
...
1. **Extraction is wrong.** `parseVariableIterable`
   (`lua/kittyREPL/commands.lua:42`) tries `pVar = "[%w_]+"` before `"%b()"`, so
...
`commands.lua:129`): python `next(x)` requires an iterator, so
...
found (the config appends `modules/*` to the rtp in `lua/dev_plugins.lua:12`),
...
1. **Kind before size.** A loop binding always outranks an assignment. Size
   alone is not enough: on a one-line loop — python `for x in y: z = w`, or the
   idiomatic R `for (i in 1:10) x <- i` — both matches start on the cursor row
   and the *assignment is the smaller span*, so innermost-wins would select the
   body statement. Verified.
2. Within a kind, among matches whose span **starts on the cursor row**: the
   first one starting at or after the cursor **column**, else the innermost span
   containing the cursor, else the innermost span on the row.
3. Otherwise, matches whose **binding span contains** the cursor → innermost.
   This is what makes a wrapped header work with the cursor on a continuation
   line.
4. else `vim.notify` a warning and do nothing (replaces the current nil-concat
   crash).

Rule 2's column preference is what lets repeated presses walk a multi-binding
header left to right (§7), and it subsumes the equal-span tiebreak that julia
...
(`total = total + i`) matches by rule 2 and yields
...
Behaviour change to accept: python `x = y = z` parses as
`assignment(x, assignment(y, z))`. The regex picked `x = y`; innermost picks
`y = z`. Chained assignment is rare and neither is more correct.

### 3. Nth-element expressions, in config

Replaces `ft_iterate` / `ft_indexing` (`commands.lua:126,138`). Three fields per
filetype, not two: `ft_indexing = {julia=1, python=0, r=1}` is *not* an indexing
style but the **default index when `vim.v.count == 0`** (`commands.lua:144`); an
...
merged by `M.setup` (`init.lua:44-54`); and §1's extensibility promise depends on
...
Task 02 adds the key-axis comment block to `config.lua` and runs first, so this
task adds its own `iterate` row to that block.

Keep the table nested per filetype rather than flattening to
`iterate_first`/`iterate_index`/`iterate_base`. `setup` merges with
`vim.tbl_deep_extend("keep", …)` (`init.lua:47`), so nesting is what lets a user
...
- **r**: `X[[i]]`. R's `for` only iterates vectors/lists, so no iterator problem
  exists. Note this makes `runLineFor` work in R for the first time — it
  currently crashes, since `ft_iterate` has no `r` entry. Intended.
...
`init.lua:112`, but the unit itself has nothing to do with REPLs).
...
`send_raw(text .. post)` (`kitty.lua:124`), bypassing the `bracketed`/`linewise`
...
costs the `custom` dispatch. `config.custom.julia` (`init.lua:74`) exists
...
line sends that line plus `post` (`kitty.lua:129-141`).

### 7. Cursor progress: advance to the next thing you would send

`config.progress` currently does `normal! j` in both commands
(`commands.lua:133,148`). Replace that, for these two commands only, with a
...
on the next binding's `@variable` start, §2 rule 2's "first binding at or after
the cursor column" consumes it exactly — the two rules compose without a
tolerance.
...
Plus a spec for `iterateExpr` (ft, key, iterable, count → string) pinning the
python `list(iter(...))` form, the `base` default when count is 0, and the
nil-template notify.
...
loop landing on `j`'s column, since that is what §2 rule 2 then consumes on the
next press.
...
- **05** gives every `kitty.lua` function an explicit window id, so the
  `kitty.run(torun)` calls here become `kitty.run(win, torun)`.
- **09** rewrites `kitty.run(` → a local `run(` wrapper throughout
  `commands.lua` and cites the ten call sites by line number
  (`09-context-sync.md:97`). This task changes that numbering — re-derive the
