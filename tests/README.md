...
`tests/fixtures/pager/` holds terminal screens captured from a live kitty,
regenerated by the sibling `pager.sh`, which documents what it needs.

`tests/fixtures/repl/` holds screens, scrollbacks and history files from a REPL
of each supported program, regenerated by the sibling `repl.sh`. Screens are read
byte for byte — the cursor escape one ends in is what a prompt is measured
against.
